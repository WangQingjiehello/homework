...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/ckpt.t8')
...
def test(model, testloader, lossfc, use_cuda, EPOCH):
    ...
    if acc > best_Acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt.t8')
        best_Acc = acc
...
def main():
    ...
    win = viz.line(
        X = np.array([x]),
        Y = np.column_stack((np.array([train_Acc]), np.array([train_loss]), np.array([test_Acc]), np.array([test_loss]))),
        opts = dict(
            legend = ["train_Acc", "train_loss", "test_Acc", "test_loss"]
            )
        )
    for i in range(start_EPOCH, 160):      
        if i % 80 == 0:
            LR = 0.0001
        print('EPOCH = %d' %i)
        for data, target in trainloader:
            ...
            if step % 781 == 0:
                ...
                test_Acc = Acc
                test_loss = loss
                print('Test: test_number = %d, loss = %.4f, current_acc = %.2f' %(test_number, loss, Acc))
            if step % 100 == 0:
                viz.line(
                    X = np.array([x]),
                    Y = np.column_stack((np.array([train_Acc]), np.array([train_loss]), np.array([test_Acc]), np.array([test_loss]))),
